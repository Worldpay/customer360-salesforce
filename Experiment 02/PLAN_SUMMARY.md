# Experiment 02 — Plan Summary

This document summarises the build plan for **Customer 360 Experiment 02**, an **LWC-only org overlay** aligned to output templates, design system, and wireframes. Experiment 01 remains frozen as reference.
...
|-----------|---------|--------|
| `c360Dashboard` | Monolithic hub — Overview, Alert Centre, Cross-Sell, Churn, Pathways, inline Account | Built (mock data) |
...
| `c360ExportModal` | R04 PowerPoint export UI stub | Built |
| `c360Account` | Account Record Page spotlight (leadSpotlight pattern) | Bootstrap (mock data) |
| `c360MockData` | Shared mock DTOs for sandbox UAT | Built |

**Not in deploy package:** Lightning App, FlexiPages, tabs, Apex classes (see `.forceignore`).

**Pilot scope implemented:** Option C (Hybrid) — `c360Dashboard` hub + Alert Centre LWCs + prototype PPT export modal retained.
...
flowchart TB
  subgraph deploy [Deploy to org]
    Dash[c360Dashboard]
    AlertC[c360AlertCentre]
    AlertD[c360AlertDetail]
    Acct[c360Account]
    Children[c360KpiTile + c360SignalList + c360ExportModal]
  end
...
- Handover deploys **LWC bundles only** — apps, flexipages, tabs, and Apex are excluded via `.forceignore` and `manifest/package-lwc.xml`.
- An admin will **swap the hub component** on the existing App Page from `c360App` (or equivalent) to `c360Dashboard` in Lightning App Builder after deploy.
...
- Alert Centre is **embedded in `c360Dashboard` navigation** (no separate flexipage required for pilot).
- Mock data runs in sandbox until org Apex is wired (Phase 5).
- `c360Account` is a **bootstrap** on the existing Account Record flexipage — full Spotlight gauge and `@wire` deferred to post–Phase 0c.
...
- Does the org use `c360App`, `c360Dashboard`, or another component name today?
...
- **Replace** `c360App` with `c360Dashboard` on the existing flexipage, or **evolve** `c360App` in the org to absorb template patterns?
...
- Should `c360Account` replace or sit alongside existing Account page components?
...
| `06. Output templates/` | `c360Dashboard` monolith, `leadSpotlight` `@wire` pattern | Child bundles, wireframe screens |
| `08. Experiments/01/` | Decomposed hybrid, illustrative metadata | Template-aligned APIs |

### Information architecture divergence

| IA element | Wireframe | Prototype | Template | Exp 02 build |
|------------|-----------|-----------|----------|--------------|
...
| Element | Wireframe hub | Prototype / template | Exp 02 |
|---------|---------------|------------------------|--------|
...
| Dimension | design-rules | 09 Design System | Prototype | Exp 02 |
|-----------|--------------|------------------|-----------|--------|
...
### Template dependency gaps (resolved in Exp 02)

| Missing from templates | Exp 02 solution |
|------------------------|-----------------|
| `c360KpiTile` child | Built with `detail` / `variant` API |
| `c360SignalList` child | Built with `signalaction` / `signaldismiss` events |
| SharePoint junk in HTML | Stripped |
| Alert Centre / Detail | New `c360AlertCentre`, `c360AlertDetail` bundles |
| Apex wiring | Mock data + `classes-stubs/APEX_MERGE_GUIDE.md` |

### Requirements coverage (Exp 02 vs sources)

| ID | Template alone | Exp 02 (Hybrid) | Gap remaining |
|----|----------------|-----------------|---------------|
...
| 3 | Port `c360Dashboard` + design tokens | **Done** |
...
| 6 | Full `c360Account` Spotlight + Record Page | **Bootstrap only** |
...
- [ORG_WIRING_RUNBOOK.md](ORG_WIRING_RUNBOOK.md) — App Builder wiring steps
- [HANDOFF_CHECKLIST.md](HANDOFF_CHECKLIST.md) — pre/post deploy verification
- [classes-stubs/APEX_MERGE_GUIDE.md](classes-stubs/APEX_MERGE_GUIDE.md) — Apex merge reference
