# Customer 360 — Plan Summary

> Duplicated from Experiment 02 as the starting point for the next experiment. Update this document as scope evolves.

This document summarises the build plan for **Customer 360 Customer 360**, an **LWC-only org overlay** aligned to output templates, design system, and wireframes. Experiment 01 remains frozen as reference.
...
|-----------|---------|--------|
| `c360Dashboard` | Monolithic hub — Overview, Alert Centre, Cross-Sell, Churn, Pathways, schema v3 Account | Built (mock data) |
...
| `c360Account` | Account Record Page — delegates to `c360AccountDetail` | Built (mock data) |
| `c360MockData` | Shared mock DTOs + `getAccountDetail()` for sandbox UAT | Built |
...
- Handover deploys **LWC bundles only** — apps, flexipages, tabs, and Apex are excluded via `.forceignore` and `manifest/packageLwc.xml`.
...
| Apex wiring | Mock data + `classesStubs/apexMergeGuide.md` |
...
- [orgWiringRunbook.md](orgWiringRunbook.md) — App Builder wiring steps
- [handoffChecklist.md](handoffChecklist.md) — pre/post deploy verification
- [classesStubs/apexMergeGuide.md](classesStubs/apexMergeGuide.md) — Apex merge reference
