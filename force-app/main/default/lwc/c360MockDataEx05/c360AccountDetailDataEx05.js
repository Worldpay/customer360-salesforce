...
export const CHURN_SCORE_LABEL = 'Predicted change in transaction count in 3-6 months';

function formatDrillValue(driverValue, index) {
    const value = String(driverValue);
    if (value === 'Yes' || value === 'No') {
        return index % 4 === 0 ? 'No' : 'Yes';
    }
    if (value === 'Rising') {
        return ['Low', 'Medium', 'High', 'Rising'][index % 4];
    }
    if (value.includes('days')) {
        const base = parseInt(value, 10) || 0;
        return `${Math.max(1, base + (index - 5))} days`;
    }
    if (value.includes('%')) {
        const base = parseFloat(value) || 0;
        const next = Math.round((base + (index - 5) * 0.7) * 10) / 10;
        return `${next}%`;
    }
    if (/^\d+$/.test(value)) {
        return String(Math.max(0, parseInt(value, 10) + (index - 5)));
    }
    return value;
}

export function expandDrillRows(driverValue, existing, idBase) {
    const rows = [];
    for (let i = 0; i < 10; i++) {
        const source = existing?.[i];
        rows.push({
            id: source?.id || source?.humanCustomerId || `HC-${idBase}${String(i + 1).padStart(3, '0')}`,
...
export function normalizeDriverDrilldown(detail, accountSeed) {
    const drill = {};
    const drivers = [...(detail.positiveDrivers || []), ...(detail.negativeDrivers || [])];
    drivers.forEach((driver, index) => {
        const existing = detail.driverDrilldown?.[driver.name] || [];
